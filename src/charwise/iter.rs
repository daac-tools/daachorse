...
fn scan<V>(
    ...
    if let Some(pf) = *prefilter {
        loop {
            ...
            // prefilter never reports positions at continuation bytes.
            let (c, char_len) = unsafe { decode_char_unchecked(haystack, *pos) };
    ...
    V: Copy,
{
    ...
    #[inline(always)]
    fn next(&mut self) -> Option<Self::Item> {
        let haystack = self.haystack.as_ref().as_bytes();
        ...
        if let Some(value) = self.pma.root_output_value() {
            ...
            if self.pos < haystack.len() {
                let (_, char_len) = unsafe { decode_char_unchecked(haystack, self.pos) };
                self.pos += char_len;
                return Some(Match {
                    length: 0,
                    end: self.pos,
